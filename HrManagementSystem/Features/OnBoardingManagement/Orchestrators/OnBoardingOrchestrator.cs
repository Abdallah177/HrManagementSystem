...
namespace HrManagementSystem.Features.OnBoardingManagement.Commands
{
    ...
    public class OnBoardingOrchestratorHandler : RequestHandlerBase<OnBoardingOrchestrator, RequestResult<bool>, Organization>
    {
        ...
        {
            var organizationResult = await _mediator.Send(new OnBoardingOrgainzationCommand(request.OnBoardingDto.OrganizationDto.Name, request.currentUserId), cancellationToken);
            //await _mediator.Send(new OnBoardingCompainesCommand());
